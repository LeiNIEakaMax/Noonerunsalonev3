...
function App() {
  ...
  const [isAddEventFormOpen, setIsAddEventFormOpen] =
    useState(false);
  ...
  const [userOverrideTheme, setUserOverrideTheme] =
    useState(false);
  ...
  useEffect(() => {
    ...
    const handleChange = (e: MediaQueryListEvent) => {
      if (!userOverrideTheme) {
        ...
        console.log(
          `💻 System theme changed: Switching to ${newTheme} mode`,
        );
        setTheme(newTheme);
      }
    };
  ...
  const handleThemeChange = (newTheme: "light" | "dark") => {
    console.log(
      `👤 User manually changed theme to: ${newTheme}`,
    );
    setTheme(newTheme);
  ...
  // If showing maintenance page, render it instead
  if (showMaintenance) {
    return (
      <MaintenancePage
  ...
  if (show404) {
    return (
      <NotFoundPage onBackToHome={() => setShow404(false)} />
    );
  }
    ...
    return (
      <div className="h-screen flex flex-col bg-brand-blue">
        ...
        <main className="flex-1 overflow-auto bg-brand-blue">
          <AboutPage5 onBack={() => setShowAbout5(false)} />
        </main>
      </div>
    );
  }
      ...
      >
        {loading ? (
          ...
          <div className="fixed inset-0 top-[60px] flex items-center justify-center bg-brand-blue">
            ...
            {/* Content Overlay */}
            <div
              className={`relative z-10 flex flex-col gap-[32px] items-start justify-center p-[32px] rounded-[0px] ${
                theme === "dark"
          ...
          <div
            className={`max-w-7xl mx-auto px-[16px] flex flex-col ${viewMode === "calendar" && !isMobile ? "h-full" : "min-h-full"} ${viewMode === "calendar" ? "py-0" : "py-0"}`}
          >
            <div
              className={`flex-1 flex flex-col min-h-0 ${viewMode === "calendar" && !isMobile ? "h-full" : ""}`}
            >
